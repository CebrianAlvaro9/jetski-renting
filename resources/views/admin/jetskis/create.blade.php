    ...
    <section style="background-image:url('{{ asset('images/jetskiCreate.jpeg') }}')"  class="bg-center bg-no-repeat bg-gray-200 bg-blend-multiply py-3 items-center justify-center  mt-3">
        <div  class="flex justify-center max-w-screen-xl mx-auto  px-4 sm:px-6 lg:px-8  ">
            <div class=" w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <form class="space-y- mb-3" method="POST" action="{{ route('admin.jetskis.store') }}" enctype="multipart/form-data">
                        @csrf
                        <h5 class="text-xl font-medium text-indigo-900 dark:text-white mb-2">Añade una moto de agua</h5>
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="horse_power" name="horse_power" value=""
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('horse_power')
                                <div class="text-sm text-red-400">{{ $message }}</div>
